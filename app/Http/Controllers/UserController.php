...
class UserController extends Controller
{
    ...
    protected static function getQueriesByStatusAndNamesPartial($operation_ids, $role_ids, $language_ids, $status, $manager_ids, $active, $dateDebut, $dateFin)
    {
        ...
        if ($status !== 'all_users') {
            $output = [
                0 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                2 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->where('active', $active)->whereIn('language_id', $language_ids)->paginate(),
                3 => User::whereNull('date_entree_production')->where('active', $active)->whereIn('language_id', $language_ids)->paginate(),
                4 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                5 => User::whereNull('date_entree_production')->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                6 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->where('active', $active)->paginate(),
                7 => User::whereNull('date_entree_production')->where('active', $active)->paginate(),
                8 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                10 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->where('active', $active)->whereIn('language_id', $language_ids)->paginate(),
                ...
                12 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                13 => User::whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                14 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->where('active', $active)->paginate(),
                15 => User::whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->where('active', $active)->paginate(),
                16 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                18 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->where('active', $active)->whereIn('language_id', $language_ids)->paginate(),
                ...
                20 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                21 => User::whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                22 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->where('active', $active)->paginate(),
                23 => User::whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->where('active', $active)->paginate(),
                24 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut, $dateFin])->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                26 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut, $dateFin])->where('active', $active)->whereIn('language_id', $language_ids)->paginate(),
                ...
                28 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut, $dateFin])->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                29 => User::whereBetween('date_entree_production', [$dateDebut, $dateFin])->where('active', $active)->whereIn('role_id', $role_ids)->paginate(),
                30 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut, $dateFin])->where('active', $active)->paginate(),
                31 => User::whereBetween('date_entree_production', [$dateDebut, $dateFin])->where('active', $active)->paginate(),
            ];
        } else {
            $output = [
                0 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                1 => User::whereNull('date_entree_production')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                2 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->whereIn('language_id', $language_ids)->paginate(),
                3 => User::whereNull('date_entree_production')->whereIn('language_id', $language_ids)->paginate(),
                4 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->whereIn('role_id', $role_ids)->paginate(),
                5 => User::whereNull('date_entree_production')->whereIn('role_id', $role_ids)->paginate(),
                6 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->paginate(),
                7 => User::whereNull('date_entree_production')->paginate(),
                8 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                10 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->whereIn('language_id', $language_ids)->paginate(),
                11 => User::whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->whereIn('language_id', $language_ids)->paginate(),
                12 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->whereIn('role_id', $role_ids)->paginate(),
                13 => User::whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->whereIn('role_id', $role_ids)->paginate(),
                14 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                15 => User::whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                16 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                18 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->whereIn('language_id', $language_ids)->paginate(),
                19 => User::whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->whereIn('language_id', $language_ids)->paginate(),
                20 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->whereIn('role_id', $role_ids)->paginate(),
                21 => User::whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->whereIn('role_id', $role_ids)->paginate(),
                22 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                23 => User::whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                24 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->paginate(),
                ...
                26 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->whereIn('language_id', $language_ids)->paginate(),
                ...
                28 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->whereIn('role_id', $role_ids)->paginate(),
                29 => User::whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->whereIn('role_id', $role_ids)->paginate(),
                30 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
    ...
    protected static function getQueriesByStatus($operation_ids, $role_ids, $language_ids, $status, $active, $dateDebut, $dateFin, $manager_ids)
    {
        ...
        if ($status !== 'all_users') {
            $output = [
                0 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                ...
                2 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                3 => User::where('active', $active)->whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                4 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereNull('date_entree_production')->paginate(),
                5 => User::where('active', $active)->whereIn('role_id', $role_ids)->whereNull('date_entree_production')->paginate(),
                6 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereNull('date_entree_production')->paginate(),
                7 => User::where('active', $active)->whereNull('date_entree_production')->paginate(),
                8 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                ...
                12 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                13 => User::where('active', $active)->whereIn('role_id', $role_ids)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                14 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                15 => User::where('active', $active)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                16 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                ...
                18 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('language_id', $language_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                ...
                20 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                21 => User::where('active', $active)->whereIn('role_id', $role_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                22 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                23 => User::where('active', $active)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                24 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
                ...
                26 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('language_id', $language_ids)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
                ...
                28 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereIn('role_id', $role_ids)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
                ...
                30 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->where('active', $active)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
                31 => User::where('active', $active)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
            ];
        } else {
            $output = [
                0 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                1 => User::whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                2 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                3 => User::whereIn('language_id', $language_ids)->whereNull('date_entree_production')->paginate(),
                4 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereNull('date_entree_production')->paginate(),
                5 => User::whereIn('role_id', $role_ids)->whereNull('date_entree_production')->paginate(),
                6 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereNull('date_entree_production')->paginate(),
                7 => User::whereNull('date_entree_production')->paginate(),
                8 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                ...
                12 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                13 => User::whereIn('role_id', $role_ids)->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                14 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                15 => User::whereDate('date_entree_production', '>=', $dateDebut ?? '1980-01-01')->paginate(),
                16 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                ...
                18 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('language_id', $language_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                19 => User::whereIn('language_id', $language_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                20 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                21 => User::whereIn('role_id', $role_ids)->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                22 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                23 => User::whereDate('date_entree_production', '<=', $dateFin ?? '2100-01-01')->paginate(),
                24 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereIn('language_id', $language_ids)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
                ...
                26 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('language_id', $language_ids)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
                ...
                28 => $users->isEmpty() ? [] : $users->toQuery()->with('operations')->with('managers')->whereIn('role_id', $role_ids)->whereBetween('date_entree_production', [$dateDebut ?? '1980-01-01', $dateFin ?? '2100-01-01'])->paginate(),
